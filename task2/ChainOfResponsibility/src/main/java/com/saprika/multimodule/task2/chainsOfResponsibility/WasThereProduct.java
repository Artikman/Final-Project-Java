...
/**
 * Class describes model of WasThereProduct
 * - implements – names of ChainsOfResponsibility.
 * @author – Artyom Sapryka.
 */
public class WasThereProduct implements ChainsOfResponsibility {

    private final static Logger logger = Logger.getLogger(String.valueOf(WasThereProduct.class));
    /**
    ...
    public void requestProcess(Value value) {
        if (!value.getValue()) {
            logger.info("Was there a product: " + value.getValue());
        } else {
